...
Class Request{
    ...
    public function submitOrderInfo(){
        ...
        $this->reqHandler->setParameter('service','pay.bestpay.jspay');//接口类型
        $this->reqHandler->setParameter('mch_id',$_POST['best_pay_id']);//必填项，商户号，由平台分配
        $this->reqHandler->setKey($_POST['best_pay_key']);
		...
		$this->reqHandler->setParameter('notify_url','http://fu99.tw/api/user_buy/best_pay_callback');//
		$this->reqHandler->setParameter('callback_url','http://fu99.tw/api/user_buy/best_pay_callback');
            ...
            if($this->resHandler->isTenpaySign()){
                ...
                if($this->resHandler->getParameter('status') == 0 && $this->resHandler->getParameter('result_code') == 0){
                    echo json_encode(array('token_id'=>$this->resHandler->getParameter('token_id'),
										...
										'pay_url'=>$this->resHandler->getParameter('pay_url')));
                    exit();
                }else{
