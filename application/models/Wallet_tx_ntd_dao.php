<?php
class Wallet_tx_ntd_dao extends MY_Model {

	function __construct() {
		parent::__construct();

		// initialize table name
		parent::set_table_name('wallet_tx_ntd');
	...
	function sum_all($corp_id) {
		$sql = "select sum(amt) as samt from $this->table_name wtx
	...
	function get_max_id($user_id) {
		$sql = "select max(id) as m_max from $this->table_name where user_id  = $user_id";
		$list = $this -> db -> query($sql) -> result();
		if(count($list) > 0) {
	...
	function find_all_corp_tx($data) {
		$s_date = $data['s_date'];
		$e_date = $data['e_date'];
		$sql = "select sum(wtx.amt) as sum_amt, br.corp_id, c.corp_name from $this->table_name wtx
	...
	function find_all_user_tx($data) {
		...
		$sql = "select * from $this->table_name wtx
	...
	function query_ajax($data, $is_sum = FALSE) {
		$start = $data['start'];
		$limit = $data['length'];
		...
		if(!$is_sum) {
			...
			$this -> db -> select('cp.corp_name');
		} else {
			$this -> db -> select('sum(_m.amt) as amt');
		}
	...
	function search_always($data) {
		...
		if(isset($data['role_id']) && ($data['role_id'] == 99 || $data['role_id'] == 1 )) {
			if($data['role_id'] == 1) {
				$this -> db -> where('_m.type_id in (12,15,19)');
				$this -> db -> where('_m.corp_id', $data['corp_id']);
			}
			if($data['role_id'] == 99) {
				$this -> db -> where('_m.type_id in (16,17,20)');
			}
		} else {
		...
		if(isset($data['search_corp_id']) && $data['search_corp_id'] > 0) {
			$this -> db -> where('_m.corp_id', $data['search_corp_id']);
		}
	...
	function ajax_from_join() {
		...
		$this -> db -> join("wallet_types_ntd wt", "wt.id = _m.type_id", "left");
		$this -> db -> join("corp cp", "cp.id = _m.corp_id", "left");
