<?php
class Baccarat_tab_safe_detail_dao extends MY_Model {

	function __construct() {
		parent::__construct();

		// initialize table name
		parent::set_table_name('baccarat_tab_safe_detail');
	...
	function random_one($winner = 0) {
		$list = $this -> db -> query("select * from baccarat_tab_safe_detail where winner = $winner order by rand() limit 1 ") -> result();
		if(count($list) > 0) {
			return $list[0];
		}
		return NULL;
	...
	function count_winner($round_id) {
		$query = $this -> db -> query("select winner, count(*) as cnt from baccarat_tab_round_detail where round_id = $round_id and status >= 5 group by  winner order by winner");
		$list = $query -> result();
		return $list;
	}
