<?php
class Transaction_record_props_dao extends MY_Model {

	function __construct() {
		parent::__construct();

		// initialize table name
		parent::set_table_name('transaction_record_props');
	...
	function transaction_record() {
		...
		$this -> db -> select("o.nick_name as seller");
		...
		$this -> db -> join("users o", "o.id = _m.shop_user_id", "left");
		$this -> db -> join("products p", "p.id = _m.product_id", "left");
		$this -> db -> where("_m.shop_user_id<>0");

		$list = $this -> db -> get() -> result();
    return $list;
  }

	function buy_record_items($data) {
		$dt = $data['dt'];
		$e_dt = $data['e_dt'];
	...
	function find_user_store($data, $is_count = FALSE) {
	...
	function find_user_buy($data, $is_count = FALSE) {
		...
		$this -> db -> select('_m.total ');
		$this -> db -> select('_m.create_time');
		$this -> db -> select('_m.shop_user_id');
