<?php
class Dt_tab_tx_dao extends MY_Model {

	function __construct() {
		parent::__construct();

		// initialize table name
		parent::set_table_name('dt_tab_tx');
	...
	function sum_by_tab_id($tab_id) {
		$list = $this -> db -> query("select sum(tx_amt) as amt from {$this->table_name} where tab_id = $tab_id") -> result();
		if(count($list) > 0 ) {
