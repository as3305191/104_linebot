<?php
class Promo_game_dao extends MY_Model {

	function __construct() {
		parent::__construct();

		// initialize table name
		parent::set_table_name('promo_game');
	...
	function update_received($pg_id) {
		$samt = $this -> sum_888amt_by_pg_id($pg_id);
		$this -> update(array(
			'received' => $samt
		), $pg_id);
	}

	function sum_888amt_by_pg_id($pg_id) {
		$sql = "select sum(amt) as samt from wallet_tx where promo_game_id = $pg_id and type_id = 800";
	...
	function find_all_paid_children($parent_game_id) {
		$this -> db -> where('parent_game_id', $parent_game_id);
		$this -> db -> where('is_sponsor', 1);
		$list = $this -> find_all();
		return $list;
	}

	function find_all_children($parent_game_id) {
		$this -> db -> where('parent_game_id', $parent_game_id);
		$list = $this -> find_all();
		return $list;
	}

	function find_all_by_user($user_id) {
	...
	function count_all_downline($user_id) {
		$this -> db -> select("count(*) as cnt");
		$this -> db -> where('intro_id', $user_id);
		$list = $this -> find_all();
		if(count($list) > 0) {
			$itm = $list[0];
	...
	function find_corp_admins($corp_id) {
		// select
		$this -> db -> select('_m.*');
		$this -> db -> select('mu.account as manager_account');
		$this -> db -> select('iu.account as intro_account');
	...
	function query_ajax($data) {
		...
		$this -> db -> select('iu.account as intro_account');
		$this -> db -> select('r.role_name');
		$this -> db -> select('ul.level_name as user_level_name, ul.thresh, ul.bonus_rate, ul.daily_withdraw_max, ul.vip_bonus, ul.birthday_bonus, ul.holiday_bonus');
		$this -> db -> select('co.corp_name, co.corp_code');
	...
	function ajax_from_join() {
		...
		$this -> db -> join("user_level ul", "ul.id = _m.user_level", "left");
	}
	...
	function nav_front_list() {
		$this -> load -> model('Nav_front_dao', 'nav_dao');
		$lv1_list = $this -> nav_dao -> find_all_by_parent_id(0);
		$sub_list = $this -> nav_dao -> find_all_not_lv1();
	...
	function change_log($user_id, $login_user_id, $tag = '') {
		$user = $this -> find_by_id($user_id);
		if(!empty($user)) {
			$data['user_id'] = $user -> id;
			$data['tag'] = $tag;
			$data['update_user_id'] = $login_user_id;
			$data['image_id'] = $user -> image_id;
			$data['bank_image_id'] = $user -> bank_image_id;
			$data['bank_image_id_2'] = $user -> bank_image_id_2;
			$data['uid_image_id'] = $user -> uid_image_id;
			$data['uid_image_id_2'] = $user -> uid_image_id_2;
			$this -> db -> insert('user_change_log', $data);
		}
	}
