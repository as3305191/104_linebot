<?php
class Rl_tab_status_dao extends MY_Model {

	function __construct() {
		parent::__construct();

		// initialize table name
		parent::set_table_name('rl_tab_status');
	...
	function find_all_order($hall_id) {
		...
		$this -> db -> join("rl_tab as tab", "tab.id = _m.tab_id", "left");
		$this -> db -> join("rl_tab_status_name as sn", "sn.id = _m.status", "left");
	...
	function get_status_name($status) {
		$name = "";
		$sql = "select * from rl_tab_status_name where id = $status";
	...
	function find_all_status_name() {
		$name = "";
		$sql = "select * from rl_tab_status_name";
		$list = $this -> db -> query($sql) -> result();
		return $list;
	}
