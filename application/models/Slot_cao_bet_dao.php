<?php
class Slot_cao_bet_dao extends MY_Model {

	function __construct() {
		parent::__construct();

		// initialize table name
		parent::set_table_name('slot_cao_bet');
	...
	function get_last_free_game_id($tab_id, $hall_id = 0, $max_id) {
		$sql = "select id from slot_cao_rounds where tab_id ={$tab_id} and hall_id ={$hall_id} and is_sp > 0 " . ($max_id > -1 ? " and id < {$max_id} " : "") .  " order by id desc limit 1";
	...
	function count_num_by_id($tab_id, $hall_id = 0, $max_id, $min_id) {
		$sql = "SELECT count(id) as samt FROM slot_cao_rounds where tab_id ={$tab_id} and hall_id ={$hall_id} and id > {$min_id}  " . ($max_id > 0 ? " and id <= {$max_id} " : "");
		$list = $this -> db -> query($sql) -> result();
		if(count($list) > 0) {
	...
	function get_last_over_100_id($tab_id, $hall_id = 0, $max_id) {
		$sql = "select id from slot_cao_rounds where tab_id ={$tab_id} and hall_id ={$hall_id} and has_100 > 0 " . ($max_id > -1 ? " and id < {$max_id} " : "") .  " order by id desc limit 1";
		$list = $this -> db -> query($sql) -> result();
		if(count($list) > 0) {
			return !empty($list[0] -> id) ? $list[0] -> id : 0;
		}
