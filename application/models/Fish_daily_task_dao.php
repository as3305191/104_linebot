<?php
class Fish_daily_task_dao extends MY_Model {

	function __construct() {
		parent::__construct();

		// initialize table name
		parent::set_table_name('fish_daily_task');
	...
	function count_user($date, $corp_id) {
		$this -> db -> select("id");
		$this -> db -> where('corp_id', $corp_id);
	...
	function query_ajax($data) {
		...
		// $this -> db -> select('iu.account as intro_account');
		$this -> db -> select('r.role_name');
		$this -> db -> select('co.corp_name, co.corp_code, co.sys_name, co.currency');
	...
	function search_always($data) {
		...
		if(isset($data['agent_lv']) && $data['agent_lv'] > -1) {
			$this -> db -> where('_m.agent_lv', $data['agent_lv']);
		}

		if(isset($data['agent_type_id']) && $data['agent_type_id'] > 0) {
			$this -> db -> where('_m.agent_type_id', $data['agent_type_id']);
		}
	}
	...
	function find_by_line_sub_and_corp($corp_id, $line_sub) {
		$list = array();
		$this -> db -> where('status', 0);
		$this -> db -> where('corp_id', $corp_id);
		$this -> db -> where('line_sub', $line_sub);
		$list = $this -> db -> get($this -> table_name) -> result();
		if(count($list) > 0) {
			return $list[0];
		}
	...
	function find_by_query_and_corp($corp_id, $query) {
		$list = array();
		$this -> db -> where('status', 0);
		$this -> db -> where('corp_id', $corp_id);
		$this -> db -> where("( nick_name like '%$query%' or account like '%{$query}%' or user_name like '%$query%' )");
		$list = $this -> db -> get($this -> table_name) -> result();
		if(count($list) > 0) {
			return $list[0];
		}
	...
	function find_all_user_by_agent_parent_user($agent_parent_user_id) {
		$list = array();
		$this -> db -> where('agent_parent_user_id', $agent_parent_user_id);
		$list = $this -> db -> get($this -> table_name) -> result();
		return $list;
	}
	...
	function find_all_line_user() {
		$this -> db -> where("line_sub <> ''");
		$list = $this -> find_all();
		return $list;
	}

	function find_by_fortune($m){
		$this -> db -> from("$this->table_name as _m");

		// select
		$this -> db -> select('_m.account, _m.sum_amt, _m.nick_name');
		$this -> db -> where("_m.is_bypass_sum_amt_rank", 0);

		$this -> db -> order_by('sum_amt','desc');
	...
	$this -> db -> select("if(count(_m.id) is null, 0, count(_m.id)) as users");
	$this -> db -> where("create_time like '$date%'");
	...
	function find_login($user_id,$getDate) {
	$this -> db -> from("$this->table_name as _m");
	$this -> db -> select('_m.*');
	...
	function on_line_reward($user_id,$getDate) {
	$this -> db -> from("$this->table_name as _m");
	$this -> db -> select('_m.*');
	$this -> db -> where("user_id",$user_id);
	...
	function find_day_mission($user_id,$getDate) {
	$this -> db -> from("$this->table_name as _m");
	$this -> db -> select('_m.*');
	$this -> db -> where("user_id",$user_id);
