<?php
class Pay_orders_dao extends MY_Model {

	function __construct() {
		parent::__construct();

		// initialize table name
		parent::set_table_name('pay_orders');
	...
	function query_ajax($data) {
		...
		// select
		$this -> db -> select('_m.*');
		$this -> db -> select('os.pay_order_status_name');
		$this -> db -> select('cm.pay_order_cate_main_name');
		$this -> db -> select('cs.pay_order_cate_sub_name');
	...
	function ajax_from_join() {
		...
		$this -> db -> join("pay_order_status as os", 'os.id = _m.status', 'left');
		$this -> db -> join("pay_order_cate_main as cm", 'cm.id = _m.pay_order_cate_main_id', 'left');
		$this -> db -> join("pay_order_cate_sub as cs", 'cs.id = _m.pay_order_cate_sub_id', 'left');
 ...
 function sum_all($data) {
	...
	function list_cate_main() {
		$sql = "select * from pay_order_cate_main";
		$list = $this -> db -> query($sql) -> result();
		return $list;
	}

	function list_cate_sub($cate_main_id) {
		$sql = "select * from pay_order_cate_sub where pay_order_cate_main_id = $cate_main_id";
		$list = $this -> db -> query($sql) -> result();
		return $list;
	}
}
