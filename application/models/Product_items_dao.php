<?php
class Product_items_dao extends MY_Model {
	...
	function find_all_by_user($user_id) {
		...
		$this -> db -> select("p.rank");
		$this -> db -> select("p.style");
		$this -> db -> select("ps.product_style_name");
		$this -> db -> from("{$this->table_name} as _m");
		$this -> db -> join("products p", "p.id = _m.product_id", "inner");
		$this -> db -> join("product_style ps", "ps.id = p.style", "left");
	...
	function find_all_style() {
		$this -> db -> from("product_style");
		$list = $this -> db -> get() -> result();
		return $list;
	}

	function find_fatal_used_count($user_id, $product_id) {
		$date = date("Y-m-d");
		$this -> db -> from("product_items_fatal_tx");
		$this -> db -> where("user_id", $user_id);
		$this -> db -> where("product_id", $product_id);
		$this -> db -> where("use_date", $date);
		$list = $this -> db -> get() -> result();
		return count($list);
	}

	function fint_all($user_id,$product_id) {
		$this -> db -> from("$this->table_name as _m");
		$this -> db -> select("if(count(_m.id) is null, 0, count(_m.id)) as total");
		$this -> db -> where("user_id", $user_id);
		$this -> db -> where("product_id", $product_id);

		$this -> db -> where("_m.is_base",0);

		$list = $this -> db -> get() -> result();
		return $list[0];
	}

	function fint_all_id_de($user_id,$product_id) {
		$this -> db -> from("$this->table_name as _m");
		$this -> db -> where("user_id", $user_id);
		$this -> db -> where("product_id", $product_id);
