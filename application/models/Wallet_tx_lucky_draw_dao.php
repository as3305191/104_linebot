<?php
class Wallet_tx_lucky_draw_dao extends MY_Model {

	function __construct() {
		parent::__construct();

		// initialize table name
		parent::set_table_name('wallet_tx_lucky_draw');
	...
	function sum_all($corp_id) {
		$sql = "select sum(amt) as samt from wallet_tx_lucky_draw wtx where wtx.corp_id = $corp_id ";
	...
	function get_max_id($user_id) {
		$sql = "select max(id) as m_max from wallet_tx_lucky_draw where user_id  = $user_id";
		$list = $this -> db -> query($sql) -> result();
		if(count($list) > 0) {
	...
	function find_all_corp_tx($data) {
		$s_date = $data['s_date'];
		$e_date = $data['e_date'];
		$sql = "select sum(wtx.amt) as sum_amt, br.corp_id, c.corp_name
		from wallet_tx_bdc wtx
	...
	function find_all_user_tx($data) {
		...
		$sql = "select * from wallet_tx_bdc wtx
	...
	function ajax_from_join() {
		...
		$this -> db -> join("wallet_types_bdc wt", "wt.id = _m.type_id", "left");
