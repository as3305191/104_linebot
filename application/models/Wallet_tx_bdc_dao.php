<?php
class Wallet_tx_bdc_dao extends MY_Model {

	function __construct() {
		parent::__construct();

		// initialize table name
		parent::set_table_name('wallet_tx_bdc');
	...
	function sum_all($corp_id) {
		$sql = "select sum(amt) as samt from wallet_tx_bdc wtx
	...
	function get_max_id($user_id) {
		$sql = "select max(id) as m_max from wallet_tx_bdc where user_id  = $user_id";
		$list = $this -> db -> query($sql) -> result();
		if(count($list) > 0) {
