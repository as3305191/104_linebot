<?php
class Wallet_tx_bkc_dao extends MY_Model {

	function __construct() {
		parent::__construct();

		// initialize table name
		parent::set_table_name('wallet_tx_bkc');
	...
	function reset_all($amt) {
		...
		foreach($user_list as $user) {
			$this -> add_sys_amt($user -> corp_id, $user -> id, $amt);
		}
	}

	function add_sys_amt($corp_id, $user_id, $amt) {
		$tx = array();
	...
	function sum_all($corp_id) {
		$sql = "select sum(amt) as samt from wallet_tx_bkc wtx
	...
	function get_max_id($user_id) {
		$sql = "select max(id) as m_max from wallet_tx_bkc where user_id  = $user_id";
		$list = $this -> db -> query($sql) -> result();
		if(count($list) > 0) {
