<?php
class Users_agent_dao extends MY_Model {
	...
	function search_always($data) {
		...
		if(!empty($data['s_account'])) {
			$this -> db -> where('_m.account', $data['s_account']);
			$this -> db -> where('_m.agent_lv', 0);// 沒有被設定過的
		} else { // 只找第二層
			$this -> db -> where('_m.agent_lv', 2);
			$this -> db -> where('_m.agent_parent_user_id', $data['parent_user_id']);
		}
