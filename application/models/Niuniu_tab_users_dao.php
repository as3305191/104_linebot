<?php
class Niuniu_tab_users_dao extends MY_Model {

	function __construct() {
		parent::__construct();

		// initialize table name
		parent::set_table_name('niuniu_tab_users');
	...
	function find_all_user_info_with_samt_by_tab_id($tab_id, $hall_id = 0) {
		...
		foreach($list as $user) {
			$samt = 0;
			if($hall_id < 0) {
				$samt = $this -> wtx_bkc_dao -> get_sum_amt($user -> user_id);
			} else {
	...
	function enter_tab($tab_id, $user_id, $hall_id) {
		$this -> load -> model('Niuniu_tab_status_dao', 'nn_tab_status_dao');
	...
	function random_one($tab_id, $hall_id) {
		$list = $this -> db -> query("select * from {$this->table_name} where tab_id = {$tab_id} and hall_id = {$hall_id} order by rand() limit 1 ") -> result();
		if(count($list) > 0) {
			return $list[0];
		}
		return NULL;
