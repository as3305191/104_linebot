<?php
class Rl_tab_dao extends MY_Model {

	function __construct() {
		parent::__construct();

		// initialize table name
		parent::set_table_name('rl_tab');
	...
	function tab_info($tab_id, $hall_id) {
		...
		tsn.status_name
		from rl_tab as _m
		left join rl_tab_status ts on ts.tab_id = _m.id and ts.hall_id = {$hall_id}
		left join rl_tab_status_name tsn on tsn.id = ts.status
		where _m.id = 1
		";
