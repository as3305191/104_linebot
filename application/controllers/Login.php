...
class Login extends MY_Base_Controller {
	...
	public function index() {
		...
		if(!empty($this -> session -> userdata('user_id'))) {
			...
			if(!empty($this -> session -> userdata('corp'))) {
				$corp = $this -> session -> userdata('corp');
				redirect($corp -> corp_code . '/login');
				return;
			} else {
				redirect("/app/#mgmt/dashboard?123");
			}
			return;
		}
		if(!empty($this -> session -> userdata('corp'))) {
	...
	public function co($corp_code) {
		...
		// check login
		if(!empty($this -> session -> userdata('user_id'))) {
			$l_user = $this -> users_dao -> find_by_id($this -> session -> userdata('user_id'));

			// check corp id
			if($l_user -> corp_id != $corp -> id) {
				// update session
