<?php
class Baccarat extends MY_Base_NoSessionController {
	...
	function __construct() {
		...
		$this -> load -> model('Baccarat_tab_round_detail_log_dao', 'btrd_log_dao');
		...
		$this -> load -> model('Baccarat_tab_round_detail_progress_dao', 'progress_dao');
	...
	public function gen_test() {
		$i_data['detail_id'] = 1;
		$i_data['status'] = 1;
		$this -> progress_dao -> insert(array(
			'detail_id' => 1,
			'status' => 1
 		));
		echo "test.......................";
		$i_data['detail_id'] = 2;
		$i_data['status'] = 2;
		$this -> progress_dao -> insert($i_data);
	}
	...
	public function gen_round_by_corp($corp_id) {
		$res = array();
		$tb_list = $this -> bc_dao -> find_all_valid_tabs_by_corp($corp_id);
		//$res['list'] = $tb_list;
		foreach($tb_list as $tb) {
			if($tb -> status == 1) {
				// stop it
	...
	function game_tick($tab_id, $res) {
		...
		$opening_waiting_seconds = OPENING_SECONDS + 30; // 停止下注 -> 開牌 (30秒後強制換局)
			...
			if(empty($a_detail)) {
				...
				if($a_detail -> status == 0) {
					...
					if($diff > $paly_seconds) { // play seconds
						$this -> progress_dao -> insert(array(
							'detail_id' => $a_detail -> id,
							'status' => ($a_detail -> status + 1)
				 		));
					...
					if(($diff > $opening_seconds && $a_detail -> is_checked == 1) || $diff > $opening_waiting_seconds) { // 停止下注 -> 開牌 (確定已經分配好才開牌)
						$this -> progress_dao -> insert(array(
							'detail_id' => $a_detail -> id,
							'status' => ($a_detail -> status + 1)
				 		));
						...
						$a_detail = $this -> btrd_dao -> find_current_round_detail($a_round -> id); // reload again
						$res[$a_round_id]['is_checked'] = $a_detail -> is_checked;
						if($a_detail -> is_checked == 0) {
							$this -> progress_dao -> insert(array(
								'detail_id' => $a_detail -> id,
								'status' => 99 // do_check only once
					 		));
							// mark as checked
							...
							if($last_result < 0) {
								...
								// log btrd_log_dao
								$sdu = array();
								$sdu['detail_id'] = $a_detail -> id;
								$sdu['player_c_0'] = $a_detail ->  player_c_0;
								$sdu['player_c_1'] = $a_detail ->  player_c_1;
								$sdu['player_c_2'] = $a_detail ->  player_c_2;
								$sdu['player_val'] = $a_detail ->  player_val;
								$sdu['banker_c_0'] = $a_detail ->  banker_c_0;
								$sdu['banker_c_1'] = $a_detail ->  banker_c_1;
								$sdu['banker_c_2'] = $a_detail ->  banker_c_2;
								$sdu['banker_val'] = $a_detail ->  banker_val;
								$sdu['winner'] = $a_detail ->  winner;
								$sdu['winner_type'] = $a_detail ->  winner_type;
								$sdu['is_changed'] = 1;
								$sdu['last_result'] = $last_result;
								$this -> btrd_log_dao -> insert($sdu);
							...
							// 最多需要三次, 而且要比上次小
							if($ret['result'] < 0 && $ret['result'] < $last_result) {
								$last_result = $ret['result'];
	...
	function check_detail($a_detail, &$res, $tab_id, $a_round_id, $is_save = FALSE) {
		...
		$total_bet = 0;
		$total_win = 0;
		$result = $tab_amt > 0 ?  $tab_amt : 0; //如果彩池是負數，則只給贏
		$result_bare = 0;
		foreach($bet_list as $a_bet) {
			...
			$a_bet -> result_bare = (floatval($a_bet -> total_bet) - floatval($user_win));// 此局彩池盈餘(此局輸贏)
			$result_bare += floatval($a_bet -> result_bare); //累計此局彩池盈餘
			$result += $a_bet -> result_bare; // 彩池剩餘總金額（加上此局）

			$bu_data['total_win'] = $user_win; // save total win
			$bu_data['sum_amt'] = $tab_amt; // save total win
			$bu_data['result_bare'] = $a_bet -> result_bare; // save total win
			...
			if($result_bare != 0) { // 這一局有輸贏
				...
				if($amt_share > 0) {
					...
					foreach($bet_list as $a_bet) {
						...
						$pool_com = $this_amt_share;
						$last_id = $a_bet -> id;

						$user = $this -> users_dao -> find_by_id($a_bet -> user_id);

						if($user -> intro_id > 0) {
							$intro_user = $this -> users_dao -> find_by_id($user -> intro_id);
							if($intro_user -> agent_lv > 0 && $intro_user -> agent_pct > 0) {
								$agent_amt = floatval($pool_com) * $intro_user -> agent_pct / 100.0;
								// wtx
								$tx = array();
								$tx['slot_bet_id'] = $last_id;
								$tx['corp_id'] = $intro_user -> corp_id; // corp id
								$tx['user_id'] = $intro_user -> id;
								$tx['amt'] = $agent_amt;
								$tx['type_id'] = 1100;
								$tx['brief'] = "會員 {$intro_user->id} {$intro_user->agent_lv}級代理 {$agent_amt} ";
								...
								$tx['user_id'] = $intro_user -> id;
								$tx['game_id'] = 1;
								$tx['type_name'] = "古墓奇兵 代理收入";
								$tx['amt'] = $agent_amt;
								$this -> agent_tx_dao -> insert($tx);

								if($intro_user -> agent_parent_user_id > 0) {
									$intro_user = $this -> users_dao -> find_by_id($intro_user -> agent_parent_user_id);
									$agent_amt = (floatval($pool_com) * $intro_user -> agent_pct / 100.0) - $agent_amt;
