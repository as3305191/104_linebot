...
class Cash_tx extends MY_Base_Controller {
	...
	public function index()
	{
		...
		$this->load->view('mgmt/cash_tx/tx_list', $data);
	}
	...
	public function get_data() {
		...
		$res['items'] = $this -> wtx_ntd_dao -> query_ajax($data);
		$res['recordsFiltered'] = $this -> wtx_ntd_dao -> count_ajax($data);
		$res['recordsTotal'] = $this -> wtx_ntd_dao -> count_all_ajax($data);

		$s_ntd = $this -> wtx_ntd_dao -> get_sum_amt($user_id);
		$res['sum_amt_ntd'] = floor($s_ntd);

		$sum_list = $this -> wtx_ntd_dao -> query_ajax($data, TRUE);
		$res['sum_amt_range'] = count($sum_list) > 0 ? (!empty($sum_list[0] -> amt) ? $sum_list[0] -> amt : 0 ) : 0;
	...
	public function edit($id) {
		...
		$this->load->view('mgmt/cash_tx/tx_edit', $data);
	}
