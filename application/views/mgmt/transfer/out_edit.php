...
<div class="jarviswidget" id="wid-id-7" data-widget-colorbutton="false"	data-widget-editbutton="false" data-widget-deletebutton="false" data-widget-sortable="false">
	<header>
		...
		<?php if(!isset($item)): ?>
			<div class="widget-toolbar pull-left">
				<a href="javascript:void(0);" id="back_parent" onclick="currentApp.doSubmit()" class="btn btn-default btn-danger">
					<i class="fa fa-save"></i>轉帳
				</a>
			</div>
		<?php endif ?>
		<?php if(isset($item) && $item -> status == 1): ?>
			<div class="widget-toolbar pull-left">
				<a href="javascript:void(0);" onclick="confirmCode()" class="btn btn-default btn-danger">
					<i class="fa fa-save"></i>確定認證碼
				</a>
			</div>
		<?php endif ?>
		<div class="widget-toolbar pull-left">
		...
		<div class="widget-toolbar pull-left">
			轉帳手續費：<span style="color: red;"><?= $config -> transfer_percent ?>％</span>
			簡訊費用：<span style="color: red;"><?= $config -> transfer_sms_amt ?></span>
		</div>
	</header>
		...
		<div class="widget-body">
			...
			<form id="app-edit-form" action="<?= base_url('mgmt/pay_records/insert_and_pay')?>" method="post" class="form-horizontal">
				...
				<?php if(isset($item) && $item -> status == 1): ?>
					<fieldset>
						<div class="form-group">
							<label class="col-md-3 control-label">請輸入認證碼</label>
							<div class="col-md-6">
								<input type="text" class="form-control" id="confirm_code" name="confirm_code" value="" />
							</div>
						</div>
					</fieldset>
				<?php endif ?>
				...
				<fieldset>
					<div class="form-group">
						<label class="col-md-3 control-label">實際轉帳金額</label>
						<div class="col-md-6">
...
<script>
	...
	function confirmCode() {
		...
		var url = baseUrl + currentApp.basePath + 'confirm_code'; // the script where you handle the form input.
		$.ajax({
			type : "POST",
			url : url,
			data : {
