			...
			<form id="app-edit-form" action="<?= base_url('mgmt/buy_products_v3/insert_and_pay')?>" method="post" class="form-horizontal">
				...
				<fieldset>
					<div class="form-group">
						...
						<div class="col-md-6">
							<select name="bet_record_id" id="bet_record_id" class="form-control" <?= isset($item) ? 'disabled="disabled"' : '' ?>>
								<?php foreach($product_list as $each): ?>
									<option value="<?= $each -> id?>" ><?=  $each -> sn . "(" . $each -> price . ")" ?></option>
								<?php endforeach ?>
							</select>
						</div>
					</div>
