							...
							<div class="widget-body no-padding">
								...
								<?php foreach($sp_rs_arr as $each):?>
									...
									<?php for($idx = 0 ; $idx < count($each -> match_arr) ; $idx++):?>
										...
										<div class="float: left;">
											...
											<table class="table table-bordered mtable" style="width: 200px;">
												<?php for($j=0; $j < 4; $j++):?>
													<tr>
													<?php for($i=0; $i < 5; $i++):?>
														<td class="<?= count($origin_coord) > $i && $origin_coord[$i] == $j ? 'active' : '' ?>"><?= $each->rs[$i][$j] ?></td>
													<?php endfor ?>
													</tr>
												<?php endfor ?>
											</table>
										...
										<div class="float: left;">
											<h5>已中獎部分: 倍數= <?= $each -> multiply_arr[$idx] ?></h5>
											<table class="table table-bordered mtable" style="width: 200px; float: left;">
												<?php for($j=0; $j < 4; $j++):?>
													<tr>
													<?php for($i=0; $i < 5; $i++):?>
...
<script type="text/javascript">
	...
	function enterFree(slotBetId) {
		$.ajax({
			type: "POST",
			url: '<?= base_url('api/slot_cao_game/enter_free_game') ?>',
			data: {
